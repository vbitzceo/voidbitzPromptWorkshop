...
public class TagsController : ControllerBase
{
    ...
    public async Task<ActionResult> CreateTag([FromBody] CreateTagRequest request)
    {
        try
        {
            ...
            if (existingTag != null)
                return Conflict($"Tag with name '{request.Name}' already exists");            var tag = new Tag
            {
                Name = request.Name,
    ...
    public async Task<ActionResult> UpdateTag(string id, [FromBody] UpdateTagRequest request)
    {
        try
        {
            ...
            var tag = await _context.Tags.FindAsync(id);
            if (tag == null)
                return NotFound($"Tag with ID {id} not found");            // Check if new name conflicts with existing tag
